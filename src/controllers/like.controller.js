...
import { ids, id, user_id, post_id, status_id } from "../helpers/joi_schema";
...
export const updateLike = async (req, res) => {
    try {
        const user_id = req.user.id;
        const { error } = joi.object({ id, user_id, status_id }).validate({user_id, ...req.body});
        if (error) return badRequest(error.details[0]?.message, res);
...
export const deleteLikes = async (req, res) => {
    try {
        ...
        if (error) return badRequest(error.details[0]?.message, res);
        const response = await services.deleteLikes(req.query);
        return res.status(200).json(response);
    }
    catch (error) {
        console.log(error)
